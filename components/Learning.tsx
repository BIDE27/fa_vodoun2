...
import { BookOpen, Video, Lock, Award, CheckCircle, Volume2, HelpCircle, ArrowRight, RefreshCw, Star, PlayCircle, GraduationCap, Clock, Monitor, Smartphone, Globe, User as UserIcon, ArrowLeft, Check, ChevronRight, MessageSquare, CornerDownRight, X, ChevronDown, List, Play, Pause, RotateCcw, RotateCw, Maximize, VolumeX, Volume1, Volume2 as Vol2Icon, Shield, Layers, Users, Wrench, ScrollText, ShoppingBag, Send, Sparkles, Zap, Calendar, Heart, Quote as QuoteIcon, CreditCard, ShieldCheck, Flame, Ghost, History as HistoryIcon, Languages, MapPin, Search, Landmark, Music, Eye } from 'lucide-react';
...
const FA_SIGNS = [
  ...
  { 
    ...
    name: 'Trukpin-Meji', 
    symbol: 'II II\nI I\nII II\nII II', 
    meaning: "Trukpin Méji (Ika-Meji) représente la Concentration, la Retenue et le secret initiatique. Il symbolise le serpent qui se mord la queue, l'infini and la protection par le cercle fermé. C'est un signe de pouvoir intérieur immense qui ne doit être utilisé qu'avec une extrême sagesse. Il gouverne le incantations (Aze) et les forces magnétiques. Trukpin Méji enseigne que la véritable force ne se montre pas, elle se contient jusqu'au moment opportun. Il demande à l'initié de cultiver la maîtrise de ses paroles et de ses émotions.", 
...
const VODOUNS_DATA = [
  ...
  },
  {
    ...
    history: "Représentée souvent sous les traits d'une sirène ou d'une femme tenant des serpents, Mami Wata gouverne l'océan et les fleuves. Elle est invoquée pour la sensualité, la guérison et le succès matériel.",
  ...
  },
  {
    ...
    history: "Aussi appelé 'Ayinon', Sakpata est le propriétaire de la terre. C'est une divinité crainte et respectée, liée à la variole autrefois, mais aujourd'hui invoquée pour la fertilité des sols et la protection collective.",
    dialect: "Fon, Adja",
  ...
  },
  {
    ...
    history: "Oro ne se voit pas, il s'entend par son cri strident (le rhombe). Il intervient lors des crises majeures de la communauté ou pour l'exécution des sentences traditionnelles.",
    dialect: "Yoruba",
  ...
  },
  {
    ...
    description: "Les enfants nés pour mourir et renaître sans cesse.",
    history: "Les Abikou font partie d'une confrérie d'esprits qui refusent de rester sur terre. Des rituels spécifiques sont pratiqués pour les attacher à la vie et rompre le cycle des décès prématurés dans une famille.",
    dialect: "Yoruba, Fon",
...
const GENERATE_CONVERSATIONS = (lang: string) => [
    { title: `Bienvenue en ${lang}`, desc: `Les salutations de base pour bien commencer en ${lang}.`, video: 'https://www.w3schools.com/html/mov_bbb.mp4' },
    ...
    { title: 'Le Corps Humain', desc: 'Vocabulaire de base pour la santé et le bien-être.', video: 'https://www.w3schools.com/html/mov_bbb.mp4' },
    ...
    { title: 'La Cuisine', desc: 'Verbes d\'action pour préparer un bon repas.', video: 'https://www.w3schools.com/html/mov_bbb.mp4' },
    ...
    { title: 'La Technologie', desc: 'Néologismes pour parler du digital.', video: 'https://www.w3schools.com/html/mov_bbb.mp4' },
...
const REVIEWS_POOL: CourseReview[] = [
    ...
    { user: 'Koffi A.', rating: 5, comment: 'Le savoir ancestral à portée de clic. Bravo pour cette plateforme !', date: 'Il y a 4 jours' },
    ...
    { user: 'Mawuna L.', rating: 4, comment: 'Indispensable pour tout Béninois de la diaspora.', date: 'Il y a 18 jours' },
    ...
    { user: 'Tchénon B.', rating: 4, comment: 'Merci pour cette transmission de qualité et cette rigueur.', date: 'Il y a 25 jours' },
    { user: 'Ayaba K.', rating: 5, comment: 'Très pédagogique, même pour les débutants complets.', date: 'Il y a 1 mois' },
    ...
    { user: 'Codjo M.', rating: 4, comment: 'Une plateforme d\'excellence pour l\'initiation graduelle.', date: 'Il y a 2 mois' },
...
const MOCK_COMMENTS: CourseComment[] = [
    ...
    },
    {
        ...
        text: 'La qualité de l\'enseignement est incroyable. Merci pour la clarté.',
...
const COURSES: Course[] = [
  ...
  { id: 'c15', title: 'Le Pouvoir des Mots (Aze)', type: 'video_paid', level: 3, price: 20000, completed: false, description: 'Apprenez à utiliser le verbe créateur et les incantations sacrées pour modifier votre réalité.', duration: '8h 00min', rating: 5.0, reviewCount: 28, trainer: MOCK_TRAINERS.expert1, benefits: ['Maîtrise de la parole de force', 'Protection par le verbe', 'Éveil de la conscience sonore'], videoUrl: 'https://www.w3schools.com/html/mov_bbb.mp4', lessons: GENERATE_LESSONS('c15'), comments: MOCK_COMMENTS, reviews: REVIEWS_POOL },
  { id: 'c16', title: 'Astronomie Ancestrale et Fa', type: 'video_free', level: 2, price: 0, completed: false, description: 'Comment les anciens observaient les astres pour synchroniser les consultations du Fa avec les cycles lunaires.', duration: '2h 45min', rating: 4.7, reviewCount: 54, trainer: MOCK_TRAINERS.expert2, benefits: ['Calendrier lunaire Fon', 'Signes et constellations', 'Agriculture et marées'], videoUrl: 'https://www.w3schools.com/html/mov_bbb.mp4', lessons: GENERATE_LESSONS('c16'), comments: MOCK_COMMENTS, reviews: REVIEWS_POOL },
...
export const Learning: React.FC<LearningProps> = ({ user, onToggleDetail, initialCourseId, onNavigate }) => {
  ...
  useEffect(() => {
    if (onToggleDetail) {
        onToggleDetail(!!selectedCourseId || isPlayingCourse || activeTab !== 'courses');
    }
  }, [selectedCourseId, isPlayingCourse, activeTab, onToggleDetail]);
      ...
      <div className="max-w-4xl mx-auto space-y-12 animate-fadeIn">
          <div className="bg-white p-10 md:p-16 rounded-[3.5rem] shadow-2xl border border-brand-50 relative overflow-hidden flex flex-col items-center">
              ...
              <div className="text-center space-y-4 mb-12">
                  ...
                  <p className="text-stone-500 text-lg max-w-xl mx-auto italic">Entrez votre nom de famille pour découvrir l'histoire de votre lignée au Bénin.</p>
              </div>
                                    ...
                                    {course.requiredTools && course.requiredTools.length > 0 ? (
                                        course.requiredTools.map((tool) => (
                                            <button 
                                                ...
                                                className="w-full flex items-center justify-between p-4 bg-brand-50 rounded-2xl border border-brand-100 hover:bg-brand-100 hover:shadow-md transition-all group"
                                            >
                                        ...
                                        <p className="text-stone-400 text-sm italic">Aucun outil spécifique n'est requis pour ce cours.</p>
                                    )}
                                </div>
                            </div>
                        </div>
    ...
    <div className="space-y-10 pb-20">
                ...
                {VODOUNS_DATA.map(vodoun => (
                    ...
                    >
                        ...
                        <div className="p-3 md:p-4">
                            ...
                            <div className="text-xs md:text-sm font-black uppercase tracking-wider">{vodoun.name}</div>
                        </div>
                    </button>
                ))}
            </div>
            <div className="lg:col-span-2 bg-white p-8 md:p-16 rounded-[3rem] shadow-2xl border border-brand-50 relative overflow-hidden flex flex-col">
                ...
                <div className="space-y-10 flex-grow overflow-y-auto no-scrollbar">
                    <section className="space-y-4">
                        <div className="flex items-center gap-3 pb-2 border-b border-stone-100">
                             <HistoryIcon size={24} className="text-brand-500"/>
      ...
      {activeTab === 'quiz' && (
          <div className="max-w-2xl mx-auto animate-slideUp">
              <div className="bg-white p-12 rounded-[3rem] shadow-2xl text-center border border-brand-50 space-y-8">
                  <div className="bg-brand-50 w-24 h-24 rounded-full flex items-center justify-center mx-auto shadow-inner"><HelpCircle size={48} className="text-brand-600" /></div>
                  <h3 className="text-3xl font-serif font-bold text-brand-900">Espace Quiz en développement</h3>
                  <p className="text-stone-500">Mettez à l'épreuve votre compréhension des traditions sacrées bientôt ici.</p>
              </div>
          </div>
      )}
    </div>
